...
def getEncoderRawValues(): #clears out the serial port and waits for the next encoder values
	...
	e0_value = 0;
	e1_value = 0;
	while (not (e0_found and e1_found)):
		try:
			...
			print "unable to find alamode.. please try again or reflash alamode"
			exit();
	return [e0_value,e1_value]
...
while not test_over:
	if (state == MOTOR_SPIN_LEFT):
		...
		txt = raw_input("did the left motor spin? (y/n)")
		if (txt == "y"):
		...
		txt = raw_input("did the right motor spin? (y/n)")
		if (txt == "y"):
		...
		txt = raw_input("did the robot move forward(ish)? (y/n)")
		if (txt == "y"):
